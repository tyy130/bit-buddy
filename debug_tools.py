...
class BitBuddyDebugger:
    ...
    def start_debug_session(self, buddy_name: str = None):
        ...
        self.session = DebugSession(
            ...
            issues=[],
            performance_metrics={},
            log_file=str([h.baseFilename for h in self.logger.handlers if hasattr(h, 'baseFilename')][0])
        )
    ...
    def _check_performance_issues(self, cpu_percent: float, memory_percent: float, process_metrics: Dict):
    ...
    def debug_file_system(self) -> Dict[str, Any]:
        ...
        debug_info = {
            "watch_directory": {
                ...
                "is_directory": self.buddy.watch_dir.is_dir() if self.buddy.watch_dir.exists() else False,
                "permissions": self._check_directory_permissions(self.buddy.watch_dir),
                "file_count": len(list(self.buddy.watch_dir.rglob("*"))) if self.buddy.watch_dir.exists() else 0
            },
            "buddy_directory": {
                "path": str(self.buddy.buddy_dir),
                "exists": self.buddy.buddy_dir.exists(),
                "files": list(self.buddy.buddy_dir.glob("*")) if self.buddy.buddy_dir.exists() else []
            },
    ...
    def debug_personality_system(self) -> Dict[str, Any]:
        ...
        debug_info = {
            ...
            "experience": {
                ...
                "recent_events": personality.experience_log[-5:] if personality.experience_log else []
            }
        }
    ...
    def run_health_check(self) -> Dict[str, Any]:
        ...
        if not health["checks"]["file_system"]["details"].get("watch_directory", {}).get("exists"):
            health["recommendations"].append("Watch directory does not exist - check path configuration")
        
        if health["checks"]["file_system"]["details"].get("rag_system", {}).get("indexed_files", 0) == 0:
    ...
    def stop_debug_session(self):
        ...
        if self.session:
            ...
            self.logger.info(f"🏁 Debug session ended: {self.session.session_id} (duration: {duration:.1f}s)")
