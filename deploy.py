...
class BuddyDeploymentManager:
    ...
    def create_buddy(self, name: str, watch_dir: Path, 
                    model_id: str = None, mesh_enabled: bool = True, debug_mode: bool = False) -> Dict:
        ...
        if not model_path.exists():
            raise FileNotFoundError(f"Model {model_id} not installed. Run: deploy.py download-model {model_id}")
...
'''
    ...
    def health_check(self) -> Dict:
        ...
        # Check dependencies
        required_packages = [
            "requests", "chromadb", "sentence-transformers", 
        ...
        try:
            ...
            if available_ram_gb < 2:
                ...
                health["recommendations"].append("Consider using TinyLlama model for low-RAM systems")
        except ImportError:
...
def main():
    ...
    try:
        if args.command == 'health':
            ...
            print(f"🎯 Buddy started! Press Ctrl+C to stop.")
            try:
                process.wait()
            except KeyboardInterrupt:
                process.terminate()
            ...
            try:
                ...
                print("❌ Test runner not available. Install with: pip install pytest pytest-asyncio")
            ...
            try:
                ...
                report = quick_debug_buddy(buddy, args.operation)
